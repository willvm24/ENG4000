...
void FRLlibNetworkArchSettings::GenerateTrainingArgs(FScriptArgBuilder& ArgBuilder) const
{
	...
	if (this->bUseLSTM)
	{
		ArgBuilder.AddFlag(TEXT("network-architecture-settings.use-lstm"));
		ArgBuilder.AddIntArg(TEXT("network-architecture-settings.lstm-cell-size"), this->LSTMCellSize);
		ArgBuilder.AddIntArg(TEXT("network-architecture-settings.max-seq-len"), this->MaxSeqLen);
	}
}
