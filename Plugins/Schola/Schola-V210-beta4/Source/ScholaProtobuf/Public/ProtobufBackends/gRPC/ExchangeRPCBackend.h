...
public:
	...
	~TExchangeRPCBackend()
	{
		UE_LOGFMT(LogScholaCommunicator, Warning, "TExchangeRPCBackend::~TExchangeRPCBackend(): Manually Deleting ExchangeRPC Backend {0}", LocalID);
		Shutdown();
		delete this->Worker;
	}
	...
	virtual void Reset() override
	{
		...
		{
			UE_LOGFMT(LogScholaCommunicator, Warning, "TExchangeRPCBackend::Reset(): Exchange {0}: Completing stale exchange on reset", LocalID);
