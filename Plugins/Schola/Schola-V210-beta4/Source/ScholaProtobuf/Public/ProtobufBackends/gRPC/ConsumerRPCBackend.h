	...
	virtual uint32 Run()
	{
		...
		while (true)
		{
			...
			if (!Status)
			{
				...
				UE_LOGFMT(LogScholaCommunicator, Warning, "TConsumerRPCWorker::Run(): Polling Queue Drained and Shutdown");
				return -1;
			}
			else if (!ok)
			{
				UE_LOGFMT(LogScholaCommunicator, Warning, "TConsumerRPCWorker::Run(): Invalid Event in Polling Completion Queue");
				...
				UE_LOGFMT(LogScholaCommunicator, Warning, "TConsumerRPCWorker::Run(): Returning From Completion Queue");
			}
			else
			{
