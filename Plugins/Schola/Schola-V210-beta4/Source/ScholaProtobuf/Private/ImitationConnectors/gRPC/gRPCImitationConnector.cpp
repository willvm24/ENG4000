...
void URPCImitationConnector::Init(const FTrainingDefinition& TrainingDefns)
{
	...
	if (!bIsReconnection)
	{
		...
		this->OnConnectorClosed.AddLambda([this]() {
			UE_LOGFMT(LogScholaProtobuf, Log, "URPCImitationConnector::Init(): Imitation Connector Closed");
		});
	...
	else
	{
		...
		UE_LOGFMT(LogScholaProtobuf, Log, "URPCImitationConnector::Init(): Server already initialized, preparing for new connection");
		...
		while (true)
		{
			...
			UE_LOGFMT(LogScholaProtobuf, Warning, "URPCImitationConnector::Init(): Drained stale start request from previous connection");
		}
		
		UE_LOGFMT(LogScholaProtobuf, Log, "URPCImitationConnector::Init(): Reset ImitationStateService for new connection");
	}
}
...
bool URPCImitationConnector::CheckForStart()
{
	...
	if (OptionalStartRequest.IsSet())
	{
		...
		for (int i = 0; i < Environments.Num(); i++)
		{
			...
			if (it != StartRequest.environments().end())
			{
				...
				if (EnvSettings.has_seed())
				{
					...
					UE_LOGFMT(LogScholaProtobuf, Log, "URPCImitationConnector::CheckForStart(): Environment {0} seeded with {1}", i, EnvSettings.seed());
				}
				...
				if (EnvSettings.options_size() > 0)
				{
					...
					UE_LOGFMT(LogScholaProtobuf, Log, "URPCImitationConnector::CheckForStart(): Environment {0} has {1} options supplied", i, EnvSettings.options_size());
				}
			}
		}
		...
		UE_LOGFMT(LogScholaProtobuf, Log, "URPCImitationConnector::CheckForStart(): Republished TrainingDefinition for new client");
	}
