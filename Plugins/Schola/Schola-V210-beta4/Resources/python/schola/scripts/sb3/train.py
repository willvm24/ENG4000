    ...
    try:
        ...
        with ScholaErrorContextManager() as err_ctxt:
            ...
            if n_sim == 1:
                ...
                # Multiple simulators: one AsyncVecEnv, one event loop, parallel async steps
            ...
            if not model_loaded:
                ...
                model: BaseAlgorithm = args.algorithm_settings.constructor(
                    policy=(
                        "MultiInputPolicy"
                        if isinstance(env.observation_space, gym.spaces.Dict)
                        else "MlpPolicy"
...
class MetaTrainSB3Command(MetaTrainCommand[SB3ScriptArgs]):
...
app = MetaTrainSB3Command(app, SB3ScriptArgs, main, logger).make()
