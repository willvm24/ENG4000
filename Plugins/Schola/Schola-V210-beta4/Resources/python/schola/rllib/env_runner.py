...
class ScholaEnvRunner(MultiAgentEnvRunner):
    ...
    def make_env(self):
        ...
        # Per-worker port: base_port + worker_index for multi-simulator
        protocol_args = dict(env_ctx.get("protocol_args", {}))
        port = protocol_args.get("port")
        if port is not None and getattr(self, "worker_index", None) is not None:
            protocol_args["port"] = port + self.worker_index

        # Create the environment
        self.env = RayVecEnv(
            env_ctx["protocol"](**protocol_args),
            env_ctx["simulator"](**env_ctx["simulator_args"]),
        )
