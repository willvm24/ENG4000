...
from .envs import DictActionBoxEnv, make_dict_action_env
from schola.scripts.sb3.train import MetaTrainSB3Command, app, main
...
def test_launch_script(make_vec_env_server, gym_id):
    ...
    args = SB3ScriptArgs(
        ...
        algorithm_settings=PPOSettings(),
    )
    main(args)
...
def test_launch_script_with_dict_action_space(make_vec_env_server):
    ...
    N_ENVS=3
    env_server_port = make_vec_env_server([make_dict_action_env(DictActionBoxEnv, False) for _ in range(N_ENVS)])
    args = SB3ScriptArgs(
        environment_settings=EnvironmentArgs(
            protocol=gRPCProtocolArgs(port=env_server_port, url="localhost")
        ),
...
def mock_main(mocker):
    ...
    return mocker.patch("schola.scripts.sb3.train.main")
...
def mock_app(mock_main):
    ...
    app = MetaTrainSB3Command(app, SB3ScriptArgs, mock_main, logger).make()
    return app.meta
...
def test_sac_with_executable_simulator(mock_app, mock_main, tmp_path):
    ...
    # Should now be UnrealExecutableSimulatorArgs with correct path
    assert isinstance(args.environment_settings.simulator, UnrealExecutableSimulatorArgs)
    assert args.environment_settings.simulator.executable_path == fake_executable



def test_ppo_with_editor_simulator(mock_app, mock_main):
    ...
    # Verify simulator is UnrealEditorSimulatorArgs and num_simulators is fixed at 1
...
def test_executable_num_simulators_parsed(mock_app, mock_main, tmp_path):
    ...
    args = mock_main.call_args[0][0]
    assert isinstance(args.environment_settings.simulator, UnrealExecutableSimulatorArgs)
    assert args.environment_settings.simulator.num_simulators == 3
