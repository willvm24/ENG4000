...
def test_rayvecenv_pettingzoo_create(make_rllib_vec_pettingzoo_env, make_pettingzoo_env):
    ...
    assert len(env.possible_agents) == 3, "PettingZoo simple_spread_v3 should have multiple agents"
    ...
    target_obs_space = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(18,))
    target_action_space = gym.spaces.Discrete(5)
...
def test_rayvecenv_pettingzoo_reset(make_rllib_vec_pettingzoo_env, make_pettingzoo_env):
    ...
    for obs in observations:
        ...
        assert len(obs) == 3, "Each environment should have 3 agents"
...
def test_rayvecenv_pettingzoo_multiple_steps(make_rllib_vec_pettingzoo_env, make_pettingzoo_env):
    ...
    env_fns = [make_pettingzoo_env("simple_spread_v3") for _ in range(3)]
    env = make_rllib_vec_pettingzoo_env(env_fns)
...
def test_rayvecenv_multiagent_training(make_vec_pettingzoo_env_server, make_pettingzoo_env, ray_cluster):
